 ...
 * the lowest h.
 * 
 * @autor Marcos Ruiz Garcia 648045
 * @author Ravi Mohan
...
public class HillClimbingSearchFlat extends NodeExpander implements Search {
	...
	public List<Action> search(Problem p) throws Exception {
		...
		Node neighbor = null;
		// loop do
		final int MAX_LLANOS = 100;
		while (!CancelableThread.currIsCanceled()) {
			List<Node> children = expandNode(current, p);
			//Quitar nodos en llano repetidos
			for(Node c:children){
				for(Node n:listaNodosLlanos){
					if(c.getState().equals(n.getState())){
						pendientesDeBorrar.add(c);
					}
				}
			}
			for(Node n:pendientesDeBorrar){
				children.remove(n);
			}
			// neighbor <- a highest-valued successor of current
			...
			else if(getValue(neighbor) == getValue(current)){
				//Añadir nodo llano
				listaNodosLlanos.add(current);
				numLlanos++;
				if(numLlanos == MAX_LLANOS){
					if (SearchUtils.isGoalState(p, current)) {
						outcome = SearchOutcome.SOLUTION_FOUND;
					}
					lastState  = current.getState();
